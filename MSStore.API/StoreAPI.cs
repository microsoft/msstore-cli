...
namespace MSStore.API
{
    public class StoreAPI : IStoreAPI, IDisposable
    {
        ...
        public StoreAPI(Configurations configurations, string clientSecret, ILogger? logger = null)
            : this(
                  configurations,
                  clientSecret,
        ...
        public async Task<PublishingStatus> PollSubmissionStatusAsync(string productId, string pollingSubmissionId, CancellationToken ct)
        {
            ...
            {
                var submissionStatus = await GetSubmissionStatusPollingAsync(productId, pollingSubmissionId, ct);
                Debug.WriteLine(JsonSerializer.Serialize(submissionStatus, submissionStatus.GetType(), SourceGenerationContext.GetCustom(true)));

                status = submissionStatus.ResponseData;
                if (!submissionStatus.IsSuccess || status == null || status.HasFailed)
                {
                    /*
                ...
                if (status.PublishingStatus is PublishingStatus.PUBLISHED or PublishingStatus.FAILED)
                {
                    Debug.WriteLine($"PublishingStatus = {status.PublishingStatus}");
                    return status.PublishingStatus;
                }
        ...
        private async Task<ResponseWrapper<SubmissionStatus>> GetSubmissionStatusPollingAsync(string productId, string submissionId, CancellationToken ct)
        {
            AssertClientInitialized();
