...
namespace MSStore.CLI.Services.Graph
{
    internal class GraphClient : IGraphClient, IDisposable
    {
        ...
        private static readonly string[] GraphUserReadScope = new[] { "https://graph.microsoft.com/User.Read" };
        ...
        public async Task<ListResponse<Organization>> GetOrganizationsAsync(CancellationToken ct)
        {
            return await InvokeAsync<ListResponse<Organization>>(
                HttpMethod.Get,
                "v1.0/organization",
                null,
                GraphUserReadScope,
