    ...
    internal class FinalizeCommand : Command
    {
        ...
        public new class Handler(ILogger<FinalizeCommand.Handler> logger, IStoreAPIFactory storeAPIFactory, TelemetryClient telemetryClient) : ICommandHandler
        {
            ...
            public async Task<int> InvokeAsync(InvocationContext context)
            {
                ...
                var flightSubmissionRollout = await AnsiConsole.Status().StartAsync("Finalizing Flight Submission Rollout", async ctx =>
                {
                    try
                    {
                    ...
                    catch (MSStoreHttpException err)
                    {
                        ...
                        {
                            ctx.ErrorStatus("Error while finalizing flight submission rollout.");
