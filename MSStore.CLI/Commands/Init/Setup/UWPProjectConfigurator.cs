...
namespace MSStore.CLI.Commands.Init.Setup
{
    internal class UWPProjectConfigurator : IProjectConfigurator
    {
        ...
        public Task<int> ConfigureAsync(string pathOrUrl, AccountEnrollment account, DevCenterApplication app, IStorePackagedAPI storePackagedAPI, CancellationToken ct)
        {
            ...
            if (properties != null)
            {
                ...
                var publisherDisplayName = properties?["PublisherDisplayName"];
                if (publisherDisplayName != null)
                {
                    publisherDisplayName.InnerText = account.Name ?? string.Empty;
                }
            }
